...
export default function App() {
    ...
    const navigate = (direction: 'next' | 'previous') => {
        if (recording || working || contacts.length < 2) return;
        const index = contacts.findIndex(row => row.id === selectedRef.current);
        const next = index < 0 ? contacts[0] : contacts[(index + (direction === 'next' ? 1 : -1) + contacts.length) % contacts.length];
        if (next && next.id !== selectedRef.current) void select(next.id);
    };
 ...
 <main className="main-panel">{error ? <div className="error" role="alert">{error}</div> : null}{adminOpen && session.role === 'admin' ? <AdminPanel key={contact ? JSON.stringify(contact) : 'empty'} contact={contact} onUpdate={updated}/> : opening ? <section className="loading">Opening contact…</section> : contact ? <ContactScreen key={session.role + ':' + contact.id} contact={contact} role={session.role} onSaved={saved} onRecording={setRecording} onNavigate={navigate} canNavigate={contacts.some(row => row.id !== contact.id)} onBusy={setWorking} onRefresh={() => select(contact.id)}/> : <section className="empty-state"><div className="eyebrow">A CLEAR CALLING LIST</div><h1>You’re caught up.</h1><p>Choose All contacts to review history or select someone to call.</p>{session.role === 'admin' ? <button className="button primary" onClick={() => setAdminOpen(true)}>Manage contacts</button> : <button className="button secondary" onClick={() => { setView('all'); setListOpen(true); }}>Browse contacts</button>}</section>}</main>
 </div></>;
}
