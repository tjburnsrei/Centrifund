...
const rpc = async (action:string,args:Record<string,any>={},session=caller) => {
 if(action==='draft.save'&&!('expectedFollowUp' in args)){
  const result=await db.query<{result:any}>("select jsonb_build_object('nextAction',s.next_action,'followUpDate',s.follow_up_date,'lastCalledAt',s.last_called_at) result from centrifund_crm.drafts d left join centrifund_crm.contact_state s on s.contact_id=d.contact_id and s.workspace_id='shared' where d.id=$1",[args.id]);
  args={...args,expectedFollowUp:result.rows[0]?.result};
 }
 return (await db.query<{result:any}>('select public.centrifund_crm_request($1,$2,$3,$4::jsonb) result',[session,version,action,JSON.stringify(args)])).rows[0].result;
};
...
beforeAll(async () => {
    ...
    for (const [token, owner, role] of [[caller, 'device-one', 'caller'], [other, 'device-two', 'caller'], [admin, 'admin-one', 'admin']]) {
        await db.query('select public.centrifund_crm_create_session($1,$2,$3,$4)', [token, owner, role, version]);
    }
    shared = randomUUID();
    privateId = randomUUID();
    phone = randomUUID();
    await db.query("insert into centrifund_crm.contacts(id,owner_workspace,name,email) values($1,'zendra','Casey Example','casey@example.invalid'),($2,'zendra','Private Example','private@example.invalid')", [shared, privateId]);
    await db.query("insert into centrifund_crm.contact_access values($1,'shared'),($1,'zendra'),($2,'zendra')", [shared, privateId]);
    await db.query("insert into centrifund_crm.phones(id,contact_id,value,normalized) values($1,$2,'202-555-0141','12025550141')", [phone, shared]);
    await db.query("insert into centrifund_crm.private_notes values($1,'zendra','PRIVATE INTERNAL NOTE')", [shared]);
}, 30000);
...
describe('database authorization and transactional saves', () => {
    ...
    it('rejects public direct database and RPC access', async () => {
        ...
        await expect(db.query("select public.centrifund_crm_request('x','x','contacts.list','{}')")).rejects.toThrow('permission denied');
    ...
    it('retries draft updates and final saves without creating duplicates', async () => {
        ...
        expect((await db.query('select * from centrifund_crm.activities where draft_id=$1', [id])).rows).toHaveLength(1);
    ...
    it('marks only the selected number bad and prevents DNC queue recycling', async () => {
        ...
        await db.query("insert into centrifund_crm.phones(id,contact_id,value,normalized) values($1,$2,'202-555-0198','12025550198')", [second, shared]);
    ...
    it('imports all valid source rows idempotently without erasing judgments', async () => {
        ...
        await db.query('update centrifund_crm.phones set is_bad=true where contact_id=$1', [imported.id]);
        await db.query("insert into centrifund_crm.contact_state(contact_id,workspace_id,do_not_call,next_action) values($1,'shared',true,'Keep this judgment')", [imported.id]);
    ...
    it('rejects expired sessions and sessions from an older password', async () => {
        await db.query("update centrifund_crm.sessions set expires_at=now()-interval '1 minute' where token_hash=$1", [other]);
        await expect(rpc('contacts.list', {}, other)).rejects.toThrow('UNAUTHORIZED');
        await expect(db.query("select public.centrifund_crm_request($1,'changed','contacts.list','{}')", [caller])).rejects.toThrow('UNAUTHORIZED');
    });
    ...
    it('orders due follow-ups before uncalled contacts and retains contacts with no phone in All', async () => {
        ...
        for (const [id, name, hasPhone] of [[due, 'ZZ Due Example', true], [uncalled, 'AA Uncalled Example', true], [missing, 'Missing Phone Example', false]] as const) {
            await db.query("insert into centrifund_crm.contacts(id,owner_workspace,name) values($1,'zendra',$2)", [id, name]);
            await db.query("insert into centrifund_crm.contact_access values($1,'shared')", [id]);
            if (hasPhone)
                await db.query("insert into centrifund_crm.phones(contact_id,value,normalized) values($1,'2025550122','12025550122')", [id]);
        }
        await db.query("insert into centrifund_crm.contact_state(contact_id,workspace_id,last_called_at,follow_up_date,next_action) values($1,'shared',now(),current_date-1,'Call')", [due]);
    ...
    it('keeps audio cleanup tombstones until late upload tokens expire', async () => {
        ...
        await db.query('select public.centrifund_crm_audio_cleanup($1)', [[id]]);
        expect((await rpc('draft.get', { id })).audio_path).not.toBeNull();
        await db.query("update centrifund_crm.drafts set updated_at=now()-interval '4 hours' where id=$1", [id]);
        await db.query('select public.centrifund_crm_audio_cleanup($1)', [[id]]);
    ...
    it('cannot clear a follow-up changed by another saved call',async()=>{
        ...
        await db.query("update centrifund_crm.contact_state set next_action='New agreed task',follow_up_date='2026-11-01' where contact_id=$1 and workspace_id='shared'",[shared]);
    ...
    it('retains completed call history through a database export and restore', async () => {
        const before = await db.query('select count(*)::int n from centrifund_crm.activities');
        const snapshot = await db.dumpDataDir('none');
        const restored = new PGlite({ loadDataDir: snapshot });
        try {
            expect((await restored.query('select count(*)::int n from centrifund_crm.activities')).rows).toEqual(before.rows);
            expect((await restored.query('select count(*)::int n from centrifund_crm.private_notes')).rows[0]).toEqual({ n: 1 });
        }
        finally {
