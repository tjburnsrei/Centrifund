...
const rpcNames={centrifund_crm_request:['p_session_hash','p_password_version','p_action','p_args','p_allow_admin'],centrifund_crm_rate_limit:['p_bucket','p_max','p_seconds'],centrifund_crm_create_session:['p_token_hash','p_owner_id','p_role','p_password_version'],centrifund_crm_audio_cleanup:['p_completed']}
...
Object.assign(process.env,{APP_ENV:'development',APP_ORIGIN:'http://localhost:5180',SUPABASE_URL:'http://127.0.0.1:54335',SUPABASE_SERVICE_ROLE_KEY:'synthetic-local-only',CALLER_PASSWORD:'demo-password',SESSION_SECRET:'synthetic-local-session-secret-for-tests-only',ADMIN_AUTH_ENABLED:'false'})
const records=[
...
for(const r of records){
 const id=randomUUID();await db.query("insert into centrifund_crm.contacts(id,owner_workspace,name,company,city,email,background) values($1,'zendra',$2,$3,$4,$5,$6)",[id,r.name,r.company,r.city,r.email,JSON.stringify({loans12:r.loans,size:325000,lender:r.lender,projects:[['Synthetic project','Richmond, VA','Example only']]})])
 await db.query("insert into centrifund_crm.contact_access values($1,'shared'),($1,'zendra')",[id])
 if(r.phone)await db.query("insert into centrifund_crm.phones(contact_id,value,normalized) values($1,$2,$3)",[id,r.phone,'1'+r.phone.replace(/\D/g,'')])
}
