...
try {
 ...
 await db.query("select public.centrifund_crm_create_session('rehearsal','rehearsal','admin','v1')")
 const rpc=async(action,args={})=>(await db.query("select public.centrifund_crm_request('rehearsal','v1',$1,$2::jsonb) result",[action,JSON.stringify(args)])).rows[0].result
 ...
 if(contact){
  ...
  if(second)await db.query("insert into centrifund_crm.contact_state(contact_id,workspace_id,do_not_call) values($1,'shared',true)",[second.id])
 }
 const snapshot=async()=>JSON.stringify((await db.query("select (select jsonb_agg(to_jsonb(a) order by id) from centrifund_crm.activities a) calls,(select jsonb_agg(to_jsonb(s) order by contact_id) from centrifund_crm.contact_state s) state,(select jsonb_agg(to_jsonb(p) order by id) from centrifund_crm.phones p) phones")).rows)
 ...
 console.log(JSON.stringify({sha256,contacts:rows.length,phones:(await db.query('select count(*)::int n from centrifund_crm.phones')).rows[0].n,withoutPhone:rows.filter(r=>!r.phones.length).length,identityReviewRows:review,repeatedImport:'passed',historyFollowupsFlags:'preserved',database:'local ephemeral rehearsal only'},null,2))
